class IsValid {
  static fullname(str) {
    ...
    if (str.length > maxSize) {
      return [true, `Per ilgas vartotojo vardas. Negalima virsyti ${maxSize} simboliu`];
    }
  ...
  static email(str) {
    ...
    const maxSize = 100;
    if (str.length > maxSize) {
      return [true, `Per ilgas email. Negalima virsyti ${maxSize} simboliu`];
    }
  ...
  static password(str) {
    ...
    if (typeof str !== 'string') {
      return [type, `Netinkamas tipas, turi buti "string"`];
    }
    if (str.length < minPasswordLength) {
      return [true, `Per trumpas password tekstas, turi buti minimum ${minPasswordLength} simboliu`];
    }
    if (str.length < maxPasswordLength) {
      return [true, `Per ilgas password tekstas, negali virsyti ${maxPasswordLength} simboliu`];
    }
